...
public class TextClusterAlgo {
	...
	static HashMap<Integer, Integer> idMap = new HashMap<Integer, Integer>(); // map
																				// between
																				// the
																				// recordId
																				// and
	...
	public void runAlgorithm(String inputPath, String outputPath,
			boolean stemFlag, boolean stopWordFlag) {
	...
	public void runAlgorithm(String inputPath, String outputPath) {
		...
		try {
			...
			if (inputPath != null) {
				...
				ArrayList<Record> records = loadInput(inputReader, stemFlag,
						stopWordFlag);
				for (Record record : records) {
					double tfIdfVector[] = new double[allWords.size()];
					int vectorIncrementer = 0;
					for (String word : allWords) {
						tfIdfVector[vectorIncrementer] = FindTFIDF(
								record.getAttribute(), word, records);
						vectorIncrementer++;
					}
				...
				for (int i = 0; i < records.size(); i++) {
					for (int j = 0; j < records.size(); j++) {
						...
						sim[i][j] = calculateSimilarity(records.get(i)
				...
				while (clusterIterator.hasNext()) {
					...
					for (int i = 0; i < list.size(); i++) {
						outputWriter.write(idMap.get(list.get(i)) + "\t"
								+ clusterNum + "\n");
					}
					clusterNum++;
				}
			...
			System.out
					.println("Either file didn't exist or error while clustering");
			e.printStackTrace();
		}
	}
	...
	/**
	 * Calculates the similarity between two documents by calculation between
	 * the vectors of the corresponding documents.
	 * 
	 * @param tfIdfVector1
	 *            tfIdf value of record 1
	 * @param tfIdfVector2
	 *            tfIdf value of record 2
	 ...
	 * 
	 * @param inputReader
	 *            the reader object to read input
	 * @param stemFlag
	 *            if true, do the stemming; else, do not stem.
	 * @param stopWordFlag
	 *            if true, do the stop word removal; else, do not remove stop
	 *            words.
	 * @return the list of records
	 */
	private static ArrayList<Record> loadInput(BufferedReader inputReader,
			boolean stemFlag, boolean stopWordFlag) {
		// TODO Auto-generated method stub
	...
	/**
	 * To find the TFIDF value for a given document and a given term in the
	 * whole set of documents
	 * 
	 * @param document
	 *            The Text record in the input file.
	 * @param term
	 *            The term in the allWords
	 * @param records
	 *            The whole record collection of the input file.
	 * @return tfidf value of the given document and the term
	 */
	private static double FindTFIDF(String document, String term,
	 ...
	 * collection i.e.; log of the ratio of total no of document in the
	 * collection to the no. of document containing the term we can also use
	 * Math.Log(occurance/(1+documentCollection.size)) to deal with divide by
	 * zero case;
	 *
	 * @param term
	 *            The term in the allWords
	...
	private static float FindInverseDocumentFrequency(String term,
		...
		return (float) Math.log((float) occurance
				/ (1 + (float) records.size()));
	}

	/**
	 * To find the ratio of no of occurance of term t in document d to the total
	 * no of terms in the document
	 *
	 * @param document
	 *            The Text record in the input file
	 * @param term
